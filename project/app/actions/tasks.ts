...
export async function createTaskAction(
	...
	if (parsed.data.assigneeId) {
		...
		if (!member)
			return {
				success: false,
				message: "The assignee must be a project member.",
			};
	}
	...
	const position =
		existing.length === 0
			? 0
			: Math.max(...existing.map((task) => task.position)) + 1;
	...
	return {
		...
		data: toBoardTask(created, assignee ?? null),
	};
}
...
export async function updateTaskAction(
	...
	return {
		...
		data: toBoardTask(updated, assignee ?? null),
	};
}
...
export async function moveTaskAction(input: {
	...
	if (
		!projectLists.some((list) => list.id === parsed.data.fromListId) ||
		!projectLists.some((list) => list.id === parsed.data.toListId)
	) {
		return {
			success: false,
			message: "The source or destination list does not exist.",
		};
	}
	...
	if (!task || task.listId !== parsed.data.fromListId) {
		return {
			success: false,
			message: "Task location changed. Refresh and try again.",
		};
	}
	...
	if (parsed.data.fromListId === parsed.data.toListId) {
		const ordered = sourceRows
			.map((row) => row.id)
			.filter((id) => id !== parsed.data.taskId);
		ordered.splice(
			Math.min(parsed.data.position, ordered.length),
			0,
			parsed.data.taskId,
		);
	...
	const destination = projectLists.find(
		(list) => list.id === parsed.data.toListId,
	);
	await db.insert(activities).values({
		projectId: parsed.data.projectId,
		...
		metadata: {
			...
			toList: destination?.name ?? "another list",
		},
	});
...
export async function bulkDeleteTasksAction(
	...
	const rows = await db
		...
		.innerJoin(lists, eq(tasks.listId, lists.id))
		.where(
			and(eq(lists.projectId, projectId), inArray(tasks.id, uniqueTaskIds)),
		);
	const permittedIds = rows.map((row) => row.id);
	if (permittedIds.length === 0)
		return { success: false, message: "No matching tasks were found." };
