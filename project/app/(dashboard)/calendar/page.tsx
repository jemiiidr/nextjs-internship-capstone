...
export default async function CalendarPage() {
	...
	for (const task of tasks) {
		const key = task.dueDate?.slice(0, 10) ?? "unscheduled";
		...
		<div className="space-y-7">
			...
			{tasks.length === 0 ? (
				<EmptyState
					...
					description="Add a due date to an assigned task and it will appear here."
				/>
			) : (
				<div className="space-y-5">
					{Array.from(groups.entries()).map(([date, items]) => (
						<section key={date}>
							<h2 className="mb-2 text-sm font-semibold uppercase tracking-wider text-blue_munsell-600 dark:text-blue_munsell-300">
								{formatDate(date)}
							</h2>
