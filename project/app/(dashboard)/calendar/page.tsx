...
function parseMonth(value?: string) {
	...
	return year >= 1 && year <= 9999 && month >= 0 && month <= 11
		? { year, month }
...
function deadlineHour(value: string | null) {
	return value ? new Date(value).getUTCHours() : 23;
}
...
export default async function CalendarPage({
	...
	const previousHref = isDayView
		? `/calendar?view=day&date=${weekDateKey(previousDay)}`
		: `/calendar?month=${monthKey(previousMonth.year, previousMonth.month)}`;
	const nextHref = isDayView
		? `/calendar?view=day&date=${weekDateKey(nextDay)}`
		: `/calendar?month=${monthKey(nextMonth.year, nextMonth.month)}`;
	const todayHref = isDayView
		? `/calendar?view=day&date=${todayKey}`
			...
			<div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_19rem]">
				<div className="min-w-0 space-y-4">
					<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
						<div className="flex min-w-0 items-center gap-2">
							<div className="flex">
								<Link href={previousHref}>
									<Button
										...
										aria-label={isDayView ? "Previous week" : "Previous month"}
									>
										<ChevronLeft size={17} />
									</Button>
								</Link>
								<Link href={nextHref}>
									<Button
										variant="secondary"
										size="icon"
										className="-ml-px rounded-l-md"
										aria-label={isDayView ? "Next week" : "Next month"}
									>
							...
							<Link href={todayHref} className="hidden sm:block">
								<Button variant="secondary">Today</Button>
							</Link>
						</div>
						<div className="grid w-full grid-cols-2 rounded-xl border border-french_gray-300 bg-white p-1 dark:border-paynes_gray-700 dark:bg-outer_space-400 sm:w-auto">
							<Link
								href={`/calendar?month=${monthKey(pickerYear, pickerMonth)}`}
								className={cn(
									"rounded-lg px-5 py-2 text-center text-sm font-medium",
									!isDayView
							...
							<Link
								href={`/calendar?view=day&date=${weekDateKey(weekAnchor)}`}
								className={cn(
									"rounded-lg px-5 py-2 text-center text-sm font-medium",
									isDayView
										? "bg-blue_munsell-500 text-white"
					...
					<section className="overflow-hidden rounded-xl border border-french_gray-300 bg-white dark:border-paynes_gray-700 dark:bg-outer_space-500">
						<div className="min-w-0">
							{isDayView ? (
								<div>
									<div className="grid grid-cols-7 border-b border-french_gray-200 px-1 py-2 dark:border-paynes_gray-700 sm:px-4 sm:py-3">
										{weekDays.map((date) => {
											const key = weekDateKey(date);
											const selected = key === selectedDayKey;
											return (
												<Link
													key={key}
													href={`/calendar?view=day&date=${key}`}
													className="group flex flex-col items-center gap-1 rounded-xl py-2 transition-colors hover:bg-platinum-100 dark:hover:bg-outer_space-400"
												>
													<span
														className={cn(
													...
													<span
														className={cn(
															"grid size-8 place-items-center rounded-full text-xs font-semibold transition-colors sm:size-9 sm:text-sm",
															selected
																? "bg-blue_munsell-500 text-white shadow-sm"
																: key === todayKey
																	? "ring-2 ring-blue_munsell-300 text-blue_munsell-600"
																	: "text-outer_space-900 group-hover:bg-white dark:text-platinum-50 dark:group-hover:bg-outer_space-300",
														)}
													>
														{date.getDate()}
													</span>
									...
									<div>
										{dayHours.map((hour) => {
											const hourTasks = selectedDayTasks.filter(
												(task) => deadlineHour(task.dueDate) === hour,
											);
											return (
												<div
													key={hour}
								...
								<>
									<div className="grid grid-cols-7">
										{WEEKDAYS.map((day) => (
											<div
												key={day}
												className="py-2 text-center text-[10px] font-semibold text-paynes_gray-500 sm:px-3 sm:py-3 sm:text-xs"
											>
										...
										{calendarDays.map((calendarDay) => {
											...
											return (
												<div
													key={key}
													className={cn(
														"min-h-16 min-w-0 border-b border-r border-french_gray-200 p-0.5 dark:border-paynes_gray-700 sm:min-h-28 sm:p-2",
														calendarDay.isCurrentMonth
															? "bg-white dark:bg-outer_space-500"
															: "bg-platinum-50/60 dark:bg-outer_space-600/40",
													)}
												>
													<span
														className={cn(
															"grid size-6 place-items-center rounded-full text-[11px] sm:size-7 sm:text-sm",
															!calendarDay.isCurrentMonth &&
																"text-french_gray-400",
															isToday &&
							...
							<div className="flex flex-wrap gap-x-5 gap-y-2 border-t border-french_gray-200 px-4 py-3 text-xs text-paynes_gray-500 dark:border-paynes_gray-700">
								<span className="flex items-center gap-2">
									<i className="size-2 rounded-full bg-rose-500" /> High
									priority
								</span>
								<span className="flex items-center gap-2">
									<i className="size-2 rounded-full bg-amber-500" /> Medium
									priority
								</span>
								<span className="flex items-center gap-2">
									<i className="size-2 rounded-full bg-emerald-500" /> Low
									priority
								</span>
							</div>
						</div>
				...
				<aside className="space-y-4">
					<section className="rounded-xl border border-french_gray-300 bg-white p-4 shadow-sm dark:border-paynes_gray-700 dark:bg-outer_space-500">
						...
						<div className="mt-3 divide-y divide-french_gray-200 dark:divide-paynes_gray-700">
							{upcoming.map((task) => {
								...
								return (
									...
									>
										<span
											className={cn(
												"grid size-11 shrink-0 place-items-center rounded-lg text-center",
												priorityCard(task.priority),
											)}
										>
											<span>
												<span className="block text-[9px] font-bold uppercase">
													{due.toLocaleString("en", {
														month: "short",
														timeZone: "UTC",
													})}
												</span>
										...
										<span className="min-w-0">
											...
											<span className="block truncate text-xs text-paynes_gray-500">
												{task.projectName} · {deadlineTime(task.dueDate)}
											</span>
										</span>
									</Link>
								);
					...
					<section className="rounded-xl border border-french_gray-300 bg-white p-4 shadow-sm dark:border-paynes_gray-700 dark:bg-outer_space-500">
						...
						<p className="mt-0.5 text-xs text-paynes_gray-500">
						...
						<div className="mt-3 space-y-1">
							{todayTasks.map((task) => (
								<Link
									key={task.id}
									href={`/projects/${task.projectId}`}
									className="flex items-center gap-2 rounded-lg py-2 text-sm hover:bg-platinum-50 dark:hover:bg-outer_space-400"
								>
									<span
										className={cn(
