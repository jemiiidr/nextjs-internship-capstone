...
export function LabelEditor({
	...
	const addLabel = () => {
		...
		setLabels((current) => [
			...current.filter(
				(label) =>
					decodeLabel(label).name.toLowerCase() !== trimmed.toLowerCase(),
			),
			encodeLabel(trimmed, color),
		]);
		setName("");
	};
		...
		<div className="space-y-2">
						...
						return (
							...
							>
								...
								<button
									...
									aria-label={`Remove ${decoded.name}`}
								>
									<X size={12} />
								</button>
							</span>
			...
			<div className="flex gap-2">
				...
				<input
					...
					className="h-10 w-12 cursor-pointer rounded-lg border border-french_gray-300 bg-white p-1 dark:border-paynes_gray-700 dark:bg-outer_space-400"
