...
export function ActivityFeed({ activities }: { activities: ActivityItem[] }) {
	return (
		<section className="rounded-xl border border-french_gray-300 bg-white p-4 dark:border-paynes_gray-400 dark:bg-outer_space-500">
			<h2 className="flex items-center gap-2 font-semibold text-outer_space-500 dark:text-platinum-500">
				<Activity size={17} /> Recent activity
			</h2>
			<div className="mt-3 space-y-3">
				{activities.length === 0 ? (
					<p className="text-sm text-paynes_gray-500">No activity yet.</p>
				) : (
						...
						<div key={item.id} className="flex gap-2">
							...
							<div>
								<p className="text-sm text-paynes_gray-500 dark:text-french_gray-400">
									<strong className="text-outer_space-500 dark:text-platinum-500">
										{item.actor.name}
									</strong>{" "}
