...
export function DashboardOverview({ stats, analytics, tasks, projects, activities, deadlines }: {
	...
	return <div className="space-y-5">
		...
		<div className="grid items-stretch gap-4 xl:grid-cols-3">
			<Card className={panelClass}><CardContent className="p-5"><PanelHeader title="My Tasks" href="/my-tasks" />{tasks.length ? <div className="divide-y divide-french_gray-200 dark:divide-paynes_gray-800">{tasks.slice(0, 5).map((task) => <Link key={task.id} href={`/projects/${task.projectId}`} className="flex items-center gap-3 py-3"><span className={cn("size-2 rounded-full", task.priority === "high" ? "bg-rose-500" : task.priority === "medium" ? "bg-amber-400" : "bg-emerald-400")}/><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{task.title}</span><span className="block truncate text-xs text-paynes_gray-500">{task.projectName}</span></span><span className="rounded-lg bg-blue_munsell-50 px-2 py-1 text-[10px] font-medium text-blue_munsell-600 dark:bg-blue_munsell-900/30 dark:text-blue_munsell-300">{task.listName}</span></Link>)}</div> : <EmptyRecords label="assigned task" />}</CardContent></Card>
